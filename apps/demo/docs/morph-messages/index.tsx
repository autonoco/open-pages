...
const threadTransition: DocTransition = {
  duration: 280,
  exit: {
    duration: 224,
    easing: EASE_IN,
...
if (typeof document !== 'undefined' && !document.getElementById('morph-messages-styles')) {
  ...
  style.textContent = [
    ...
    `@keyframes morph-messages-doc-left { from { transform: translateX(${960 - DIAMOND_X}px); } to { transform: none; } }`,
    '@keyframes morph-messages-doc-right { from { opacity: 0; transform: translateX(-64px); } to { opacity: 1; transform: none; } }',
  ...
  <section style={stage}>
    <div style={thread}>
      ...
      <DebutLine
        ...
        background="var(--osd-accent)"
      >
        Use the new Morph Transition primitive from open-pdf
      </DebutLine>
    </div>
  </section>
);
...
// visible). While the morph runs, the fill-both doc-left animation holds
// the wrapper offset so the diamond sits at canvas center — the morph
// measures and lands there — then docs it left as the command sweeps in.
const Spin: Page = () => {
  const animate = useIsActivePage();
  return (
    <section style={stage}>
      <div
        style={{
          ...
          inset: 0,
          animation: animate
            ? `morph-messages-doc-left ${DOC_MS}ms ${EASE_OUT} ${HERO_MORPH_MS}ms both`
            : 'none',
        }}
      >
        <MorphElement id="msg-introducing">
      ...
      <div
        style={{
          ...
          color: 'var(--osd-text)',
          animation: animate
            ? `morph-messages-doc-right ${DOC_MS}ms ${EASE_OUT} ${HERO_MORPH_MS + 140}ms both`
            : 'none',
        }}
      >
