import { type DesignSystem, type DocMeta, type Page, useDocPageNumber } from '@open-pdf/core';
  ...
  return (
    ...
    >
      <span>
        ...
        <span style={{ color: text2 }}>open-pdf</span>
      </span>
...
const Agenda: Page = () => {
  const items: { n: string; label: string; tint: string }[] = [
    ...
    { n: '02', label: 'The file contract — what every doc exports', tint: 'var(--osd-accent)' },
  ...
  const lines: { code: ReactNode; tag?: string; tagColor?: string }[] = [
    {
      code: (
        <>
          ...
          <Token c={warm}>Page</Token>, <Token c={warm}>DocMeta</Token> {'}'}{' '}
          <Token c={violet}>from</Token> <Token c={mint}>'@open-pdf/core'</Token>
          {';'}
        </>
      ),
    },
    ...
    { code: <>&nbsp;</> },
    {
      code: (
        <>
          ...
          <Token c={warm}>DocMeta</Token> = {'{'} title: <Token c={mint}>'My doc'</Token> {'}'}
          {';'}
        </>
      ),
      tag: 'header label',
  ...
  return (
    <div style={{ ...fill, padding: '120px 100px' }}>
      ...
      <p style={{ fontSize: 30, color: text2, lineHeight: 1.5, margin: 0, maxWidth: 1600 }}>
        ...
        under your <code style={{ fontFamily: 'var(--osd-font-display)', color: mint }}>docs/</code>{' '}
      ...
      >
        ...
        >
          ...
          <div style={{ color: 'var(--osd-accent)' }}>virtual:open-pdf/docs</div>
          <div style={{ height: 14 }} />
...
const VirtualModules: Page = () => {
  const mods = [
    {
      id: 'virtual:open-pdf/docs',
      tint: 'var(--osd-accent)',
      bullets: ['docIds: string[]', 'loadDoc(id) → import(...)', 'one async chunk per doc'],
    },
    {
      id: 'virtual:open-pdf/config',
      tint: mint,
      bullets: ['runtime feature flags', 'showDocBrowser, showDocUi', 'allowHtmlDownload'],
    },
    {
      id: 'virtual:open-pdf/folders',
      tint: warm,
      bullets: ['reads .folders.json', 'folder tree + doc → folder map', 'powers the sidebar UI'],
    },
  ];
  return (
    <div style={{ ...fill, padding: '120px 100px' }}>
...
const VitePluginHooks: Page = () => {
  const hooks = [
    ...
    { name: 'resolveId()', desc: 'claim the three virtual:open-pdf/* ids', tint: mint },
    { name: 'load()', desc: 'generate docIds + loadDoc() switch on the fly', tint: warm },
    {
      name: 'configureServer()',
      desc: 'watch docs/* — invalidate vmod, full-reload',
      tint: violet,
    },
  ];
  return (
...
const HotReload: Page = () => {
  const steps = [
    { time: 't = 0', label: 'fs writes docs/cover/index.tsx', tint: muted },
  ...
  return (
    <div style={{ ...fill, padding: '120px 100px' }}>
      ...
      <div style={{ height: 24 }} />
      <p style={{ fontSize: 30, color: text2, lineHeight: 1.5, margin: 0, maxWidth: 1600 }}>
        Edit a doc file → <span style={{ color: 'var(--osd-accent)' }}>Fast Refresh</span>. Add or
        remove a doc folder → the watcher invalidates{' '}
        <code style={{ fontFamily: 'var(--osd-font-display)', color: mint }}>
          virtual:open-pdf/docs
        </code>{' '}
...
const DesignPanelWrite: Page = () => {
  const ring = [
    ...
    { label: 'HMR re-runs doc', tint: warm, sub: 'design const = new value' },
  ];
  return (
    <div style={{ ...fill, padding: '120px 100px' }}>
      <style>{keyframes}</style>
      ...
      >
        ...
        >
          ...
          >
            ...
            {'}'}
            {'\n  '}
            <span style={{ color: mint }}>data-pdf-loc</span>=
            <span
              style={{
                color: warm,
        ...
        >
          {[
            { n: '01', label: 'click "Inspect" in the doc header', tint: 'var(--osd-accent)' },
...
const Cli: Page = () => {
  const cmds = [
    ...
    { cmd: 'open-pdf build', desc: 'viteBuild() → static SPA in dist/', tint: mint },
    { cmd: 'open-pdf preview', desc: 'serve dist/ for local check', tint: warm },
    {
      cmd: 'npx @open-pdf/cli init',
  ...
  return (
    <div style={{ ...fill, padding: '120px 100px' }}>
      ...
      >
        <span style={{ color: 'var(--osd-accent)' }}>›</span> per-doc chunks come for free from{' '}
...
export const meta: DocMeta = { title: 'Inside open-pdf', createdAt: '2026-05-05T17:45:52Z' };
