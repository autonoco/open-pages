...
![Open-doc inspector with the property panel docked beside the canvas, editing a selected element's typography and colour](/assets/screenshots/inspector.webp)

The inspector is a dev-only overlay built into the open-pdf runtime. Toggle
it from the toolbar (or press `i`), then click any element on the doc.

## Two surfaces

The inspector exposes two ways to edit a doc:
...
The inspector pins the note as an `@pdf-comment` marker in the source. The
...
your agent reads via [`/current-doc`](/docs/skills/current-doc) — so
