---
title: Overview
description: How open-pdf and your coding agent work together.
---

open-pdf treats your coding agent as a first-class user: every workspace
ships with agent rules and a set of skills the agent invokes by name.

## What ships in the workspace

`npx @open-pdf/cli init` generates an `AGENTS.md` (and `CLAUDE.md`) at the
root with the framework's hard rules, plus five skills under
`.agents/skills/` (with `.claude/skills/` symlinks for Claude Code):

- [`/create-doc`](/docs/skills/create-doc)
- [`/doc-authoring`](/docs/skills/doc-authoring)
- [`/apply-comments`](/docs/skills/apply-comments)
- [`/create-theme`](/docs/skills/create-theme)
- [`/current-doc`](/docs/skills/current-doc)

The skills are kept inside the workspace (not the agent's global config) so
they version with the project. `open-pdf sync:skills` re-syncs them from
the package if you upgrade.

## The hard rules (`AGENTS.md`)

- Docs go under `docs/<kebab-case-id>/`.
- Entry is `docs/<id>/index.tsx`.
- Assets sit in `docs/<id>/assets/`.
- Don't touch `package.json`, `open-pdf.config.ts`, or other docs.
...
workspace can author docs — Claude Code, Codex, Cursor, Gemini CLI,
...
`doc-authoring` skill into context manually and ask the agent to follow
its rules.
