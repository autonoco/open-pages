...
- `themes/<id>.md` — palette, type stack, layout vocabulary, fixed
  components, motion, and voice notes that `/create-doc` reads on its
  next run.
- `themes/<id>.demo.tsx` — a runnable 2–3 page mini-doc that the dev UI's
...
- **An existing deck.** The skill scans `docs/<id>/index.tsx`, distils the
...
Once the bundle exists, `/create-doc` offers the theme on its next run —
