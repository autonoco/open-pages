---
title: /doc-authoring
description: The technical reference the agent reads before writing a doc.
---

`/doc-authoring` is the canonical spec for everything inside
`docs/<id>/` — the file contract, the 1920×1080 canvas, type scale,
spacing, palette rules, and per-primitive references. `/create-doc` and
`/apply-comments` defer to it for the *how*.

You don't usually invoke `doc-authoring` directly. Other skills do.
