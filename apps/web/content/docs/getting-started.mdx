...
```npm
npx @open-pdf/cli init my-deck
cd my-deck
npm run dev
```

The scaffolder generates a workspace (abridged — see
[open-pdf init](/docs/cli/init) for the full tree):

```text
my-deck/
├── docs/
│   └── getting-started/    # your first doc
├── themes/                 # shared design systems
├── .agents/skills/         # bundled agent skills
├── open-pdf.config.ts    # framework config
...
3. Save. Comments are persisted as `@pdf-comment` markers in the source.
