...
- **`@autono/open-pdf`** provides everything else (`dev`, `build`, `preview`, `export`, `sync:skills`, `update`). It is a dependency of every workspace, so inside a workspace `npx open-pdf <command>` resolves to it. The scaffolded `package.json` also maps npm scripts: `npm run dev`, `npm run build`, `npm run preview`, `npm run export`, `npm run sync:skills`, `npm run update`.
...
Update `@autono/open-pdf` in this workspace to the latest version on npm, then sync the skills it ships.
...
`update` checks npm's `latest` tag against the installed version and exits early if there is nothing to do. Otherwise it runs your package manager's install command (`npm install @autono/open-pdf@latest`, or the `pnpm` / `yarn` / `bun` equivalent, detected from the lockfile) and then `open-pdf sync:skills` from the freshly installed package. Your `docs/`, `open-pdf.config.ts`, and everything else in the workspace are untouched.

The same update is available from the dev server: when a newer version exists, the home page shows an **Update** button. Restart `open-pdf dev` after either path.
