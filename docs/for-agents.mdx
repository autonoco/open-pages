...
`npx @open-pdf/cli init` installs five skills into `.agents/skills/` and symlinks each into `.claude/skills/`, so Claude Code picks them up natively while other agents read them from the neutral `.agents/` location.
...
Skills are managed by `@open-pdf/core`. Do not edit them in place; local edits are overwritten on the next sync. To pull the latest versions:
