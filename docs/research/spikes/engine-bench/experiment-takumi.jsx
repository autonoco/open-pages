    ...
    <td tw="p-2">
      <div tw="flex flex-col">
        ...
        <span tw="text-[10px] text-slate-500">
          Wrapping detail sentence one for the row. Wrapping detail sentence two for the row so the
          row spans several lines and can straddle a page boundary.
        </span>
      </div>
    </td>
  </tr>
...
await writeFile(
  'exp-b-breakinside.pdf',
  await render(
    <table tw="w-full text-[13px]" style={{ borderCollapse: 'collapse' }}>
      <thead>
        <tr tw="bg-slate-100 font-bold">
          <th tw="p-2 text-left">ID</th>
          <th tw="p-2 text-left">Description</th>
        </tr>
      </thead>
      <tbody>{rows}</tbody>
    </table>,
