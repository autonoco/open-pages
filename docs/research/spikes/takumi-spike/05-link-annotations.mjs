...
const doc = h(
  ...
  h(
    "a",
    { href: loc(8, 5), style: { display: "flex", justifyContent: "space-between", padding: 12, border: "1px solid #999" } },
    h("span", {}, "boxed left"),
    h("span", {}, "boxed right"),
  ),
  // force page 2
...
const pdoc = await getDocument({ data: new Uint8Array(await readFile(new URL("./out-links.pdf", import.meta.url))) }).promise;
for (let p = 1; p <= pdoc.numPages; p++) {
  const page = await pdoc.getPage(p);
  const annots = await page.getAnnotations();
  console.log(`PAGE ${p}:`);
  for (const a of annots) {
    console.log("  ", a.subtype, "rect=", a.rect.map((n) => +n.toFixed(1)), "url=", a.url);
  }
}
await pdoc.cleanup();
