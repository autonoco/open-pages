...
export const DOC_ID_RE = /^[a-z0-9_-]+$/i;
...
export function validateDocName(v: unknown): string | null {
...
export async function rmDocDir(docsRoot: string, docId: string): Promise<boolean> {
  if (!DOC_ID_RE.test(docId)) return false;
  const dir = path.resolve(docsRoot, docId);
  if (!dir.startsWith(docsRoot + path.sep)) return false;
  try {
...
export async function duplicateDocDir(
  docsRoot: string,
  docId: string,
  desiredId?: string,
): Promise<{ ok: true; docId: string } | { ok: false; status: number; error: string }> {
  if (!DOC_ID_RE.test(docId)) return { ok: false, status: 400, error: 'invalid docId' };

  const root = path.resolve(docsRoot);
  const srcDir = path.resolve(root, docId);
  if (!srcDir.startsWith(root + path.sep)) {
    return { ok: false, status: 400, error: 'invalid docId' };
  }

  try {
    await fs.access(path.join(srcDir, 'index.tsx'));
  } catch {
    return { ok: false, status: 404, error: 'doc not found' };
  }

  let newId: string;
  if (desiredId !== undefined) {
    if (!DOC_ID_RE.test(desiredId)) return { ok: false, status: 400, error: 'invalid newId' };
    newId = desiredId;
    ...
    try {
      ...
      return { ok: false, status: 409, error: 'doc already exists' };
    } catch {}
  } else {
    let suffix = 1;
    while (true) {
      newId = suffix === 1 ? `${docId}-copy` : `${docId}-copy-${suffix}`;
      try {
  ...
  try {
    ...
    if (metaTitle.kind === 'unsupported') {
      return { ok: false, status: 422, error: 'could not update copied doc title' };
    }
    const title = metaTitle.kind === 'found' ? metaTitle.title : docId;
...
export function resolveDocEntry(docsRoot: string, docId: string): string | null {
  ...
  const dir = path.resolve(docsRoot, docId);
  if (!dir.startsWith(docsRoot + path.sep)) return null;
  // The DocMeta contract says every doc has docs/<id>/index.tsx; we only
...
/**
 * Rewrite (or insert) the `title` field in the doc module's `export const meta`.
 *
 * Strategy:
...
export function updateMetaTitleInSource(source: string, title: string): string | null {
  ...
  const insertion = `export const meta: DocMeta = { title: ${newLiteral} };\n\n`;
