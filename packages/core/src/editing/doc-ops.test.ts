...
describe('duplicateDocDir', () => {
  it('duplicates a doc directory with an automatic copy id', async () => {
    await withDocsRoot(async (root) => {
      await writeDoc(root, 'cover', 'Cover');

      const result = await duplicateDocDir(root, 'cover');

      expect(result).toEqual({ ok: true, docId: 'cover-copy' });
  ...
  it('increments the automatic copy id when a copy already exists', async () => {
    await withDocsRoot(async (root) => {
      ...
      expect(await duplicateDocDir(root, 'cover')).toEqual({ ok: true, docId: 'cover-copy' });
      expect(await duplicateDocDir(root, 'cover')).toEqual({
        ok: true,
        docId: 'cover-copy-2',
      });
    });
  ...
  it('rejects source doc ids with bad characters', async () => {
    await withDocsRoot(async (root) => {
      expect(await duplicateDocDir(root, 'bad id')).toMatchObject({ ok: false, status: 400 });
    });
  });
  ...
  it('rejects an existing desired id', async () => {
    await withDocsRoot(async (root) => {
      ...
      expect(await duplicateDocDir(root, 'cover', 'target')).toMatchObject({
        ok: false,
        status: 409,
      });
    });
  ...
  it('rejects path traversal in the source doc id', async () => {
    await withDocsRoot(async (root) => {
      expect(await duplicateDocDir(root, '..')).toMatchObject({ ok: false, status: 400 });
    });
  });

  it('returns not found when the source doc does not exist', async () => {
    await withDocsRoot(async (root) => {
      expect(await duplicateDocDir(root, 'missing')).toMatchObject({ ok: false, status: 404 });
    });
  });
});

describe('validateDocName', () => {
  it('accepts longer doc names than folder names', () => {
    expect(validateDocName('x'.repeat(80))).toBe('x'.repeat(80));
    expect(validateDocName('x'.repeat(81))).toBeNull();
  });

  it('rejects empty input', () => {
    expect(validateDocName('')).toBeNull();
    expect(validateDocName('   ')).toBeNull();
  });
});
...
describe('updateMetaTitleInSource', () => {
  it('replaces an existing single-quoted title literal', () => {
    const source = `export const meta: DocMeta = { title: 'old' };\nexport default [];\n`;
  ...
  it('injects a fresh meta export when none exists', () => {
    ...
    expect(out).toContain("export const meta: DocMeta = { title: 'fresh' };");
