...
// revisits within the same tab skip straight to the docs.
const warmedDecks = new Set<string>();

export function isDeckWarmed(docId: string): boolean {
  return warmedDecks.has(docId);
}

export function markDeckWarmed(docId: string): void {
  warmedDecks.add(docId);
}
 ...
 *
 * Pages mount a few per frame to keep the first doc's paint smooth, stay
 ...
 */
export function DocPreloadLayer({ pages, index, design, includeCurrent = false, onDone }: Props) {
  ...
  // under a reused component instance (a client-side doc switch keeps this
    ...
    >
      {order.slice(0, mountedCount).map((pageIndex) => {
        ...
        return (
          ...
          >
            <DocPageProvider index={pageIndex} total={pages.length}>
              <StepHost
                isActivePage={false}
