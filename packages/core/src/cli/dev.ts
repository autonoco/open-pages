...
async function startServer(opts: DevOptions): Promise<void> {
  ...
  if (process.send && address && typeof address === 'object') {
    process.send({ type: 'open-pages:listening', port: address.port });
  }
}
...
function supervise(opts: DevOptions): void {
  ...
  const spawnChild = () => {
    ...
    child.on('message', (message) => {
      ...
      if (type === 'open-pages:listening' && typeof reportedPort === 'number') {
        port = reportedPort;
      }
    });
...
function resolveDevEntry(userCwd: string): string {
  ...
  const installed = path.join(userCwd, 'node_modules', '@autono', 'open-pages', 'bin.js');
