...
//
// When the notes plugin writes a doc file in response to a speaker-note
// edit, that file change fires an HMR update. Two plugins need to recognise
// the write as self-induced so they can leave the doc canvas alone:
//   - notes-plugin suppresses React Fast Refresh (RFR remounts the tree and
//     steals textarea focus mid-typing);
//   - open-pdf-plugin must NOT broadcast `open-pdf:doc-changed`, which
//     bumps the dev `loadDoc` cache-bust token and forces a module reload
//     and remount.
//
