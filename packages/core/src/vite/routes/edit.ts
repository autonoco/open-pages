...
// POST /__edit                applyEdit({ pageId, line, column, ops })
// POST /__edit/revert-asset   applyRevertAsset({ pageId, assetPath })
  ...
  server.middlewares.use('/__edit', async (req, res, next) => {
    ...
    try {
      ...
      if (url.pathname === '/revert-asset') {
        const body = (await readBody(req)) as { pageId?: string; assetPath?: string };
        const pageId = body.pageId ?? '';
