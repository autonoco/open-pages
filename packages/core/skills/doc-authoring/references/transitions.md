# Page transitions (`DocTransition`)

The framework can run an enter/exit animation between every doc change. There's **no default** — pages snap unless you declare a `DocTransition`. Snap-swap is a perfectly tasteful default; only opt in when motion adds something.
...
Use this set as a starting point. Pick one as the deck's house transition; optionally reserve a second for hero/cover docs and a third for genuine section breaks. The CSS-`calc` + `--osd-dir` trick lets a single definition mirror itself on backward navigation when needed.
...
Most tasteful tools don't mirror on backward navigation. When you genuinely need to — e.g. a horizontal doc that should reverse — use `--osd-dir` inside `calc()`:
...
- ❌ `translateX(100%)` doc-from-side — iOS modal / PowerPoint Push; not a doc change.
...
- ❌ Duration > 350 ms for a standard doc change — drags.
