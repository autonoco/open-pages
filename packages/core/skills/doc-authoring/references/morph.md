...
> Morph builds on the `DocTransition` contract — read `transitions.md` first if you haven't.
...
```tsx
import { MorphElement, type Page, type DocTransition } from '@open-pdf/core';
import type { CSSProperties } from 'react';

// Morph transition — opacity-only enter/exit keeps all the motion on the clones (see rules).
const morphTransition: DocTransition = {
  duration: 280,
...
1. **Prefer opacity-only enter/exit on morphing pages.** Any transition family composes correctly with the morph (shared rects are measured before the phases start), but while clones glide, a transform-bearing enter also docs the rest of the page — two competing motions. Giving the clones all the motion and fading everything else is what makes a morph read as one confident gesture.
