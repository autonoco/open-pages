---
name: current-doc
description: Resolve which doc, page, and (optionally) selected element the user is currently viewing in the open-pdf dev server. Consult this whenever the user references "this page", "this doc", "this element", "the doc I'm on", "the current page", or any deictic reference to doc content without naming it. Re-read `node_modules/.open-pdf/current.json` at the start of every such turn — the user navigates between turns, so a value you read earlier in the conversation is almost certainly stale.
---

# Where is the user right now?

When the user says "fix this page", "tweak this heading", or "the doc I'm looking at", they almost never name the doc id, page number, or element — they mean wherever they are in the dev viewer. Before asking "which doc?" or "which element?", check the file the dev server writes on every navigation and inspector pick.

## Re-read on every deictic turn — never reuse a prior read

`current.json` is a live cursor, not a fact about the conversation. The user moves between docs, pages, and elements freely between your turns — including while you were doing other work. **Read the file fresh at the start of every new turn that uses a deictic reference**, even if:

- you already read it earlier in this same conversation,
- you just finished editing the doc it pointed to,
- the user's new message sounds like a continuation ("now make it bigger", "also fix this one", "keep going").

A "continue editing" follow-up is exactly the case where the user has likely just navigated to a different doc or picked a different element. Trusting your last read here will silently edit the wrong file. Re-read, compare `docId` / `pageIndex` / `selection` against what you used last time, and act on the new values.
...
- The user references the current doc/page deictically: "this", "here", "the page I'm on", "the doc I'm looking at", "what I'm working on".
...
- Before asking "which doc?" or "which element?" as a clarifying question — check this file first.
- Before guessing from `git log`, recently-edited files, or the most recent doc folder.

## When NOT to use this

- The user names a doc explicitly ("edit `q2-roadmap`") — use that name directly.
- The `apply-comments` workflow already finds the right file via `@pdf-comment` markers; it doesn't need this skill.
- For listing or discovering docs — read `docs/` directly.
...
- **Hours/days old**: ignore it. Ask the user which doc they mean.
