...
test.describe('command menu', () => {
  test('opens on the home page and jumps to a page', async ({ page }) => {
    await page.goto('/');
    ...
    await page.getByRole('option', { name: 'Pricing Page' }).click();
    await expect(page).toHaveURL(/\/p\/pricing$/);
  });
  ...
  test('switches theme from the shared appearance group', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Open command menu' })).toBeVisible();
    await page.keyboard.press('ControlOrMeta+k');
    await page.getByPlaceholder('Search pages or run a command').fill('theme dark');
