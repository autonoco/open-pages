// Boots `open-pages dev` for the e2e suite against a throwaway copy of the
...
if (!existsSync(path.join(coreRoot, 'dist', 'cli', 'bin.js'))) {
  console.error(
    'packages/core/dist is missing. Run `pnpm --filter @autono/open-pages build` first.',
  );
  process.exit(1);
}
...
const child = spawn(
  ...
  ],
  {
    cwd: scratchDir,
    stdio: 'inherit',
    // OPEN_PAGES_DEV_SUPERVISED=1 skips the CLI's supervisor fork so Playwright
    // manages a single server process. The skills drift check is already
    // disabled by the --no-skills-check flag above.
    env: { ...process.env, OPEN_PAGES_DEV_SUPERVISED: '1' },
  },
);
